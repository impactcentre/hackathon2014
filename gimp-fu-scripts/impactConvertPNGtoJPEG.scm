...
(define (impactConvertPNGtoJPEG infile outfile quality smoothing optimize progressive)
	(let* ((image (car (file-png-load 1 infile infile)))
	 ...
	 (file-jpeg-save 
		 ...
		 quality     ;   quality         FLOAT    quality [0,1] - Note: Gimp only takes 
					 ;     1st position after decimal point into account. 
...
(script-fu-register
   "impactConvertPNGtoJPEG"
   "<Toolbox>/Xtns/Script-Fu/impact/impactConvertPNGtoJPEG"
   "impact Convert PNG to JPEG" 
   "IMPACT"
