...
(define (impactConvertJPEGtoGIF infile outfile interlace dither palette num-colors alpha-dither remove-unused)
	(let* ((image (car (file-jpeg-load 1 infile infile)))
	 ...
	 (if (gimp-drawable-is-rgb drawable)
...
(script-fu-register
   "impactConvertJPEGtoGIF"
   "<Toolbox>/Xtns/Script-Fu/impact/impactConvertJPEGtoGIF"
   "impact Convert JPEG to GIF" 
   "IMPACT"
