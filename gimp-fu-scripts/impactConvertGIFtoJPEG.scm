...
(define (impactConvertGIFtoJPEG infile outfile quality smoothing optimize progressive)
	(let* ((image (car (file-gif-load 1 infile infile)))
		(drawable (car (gimp-image-active-drawable image)))
	 )
	 
	 ; convert indexed to rgb if indexed is not supported by the target format
...
(script-fu-register
   "impactConvertGIFtoJPEG"
   "<Toolbox>/Xtns/Script-Fu/impact/impactConvertGIFtoJPEG"
   "impact Convert GIF to JPEG" 
   "IMPACT"
