...
(define (impactConvertPStoTIFF infile outfile num-colors compression)
	(let* ((image (car (file-ps-load 1 infile infile)))
	 ...
	 (file-tiff-save
		 ...
		 compression  ;   compression  INT32     Compression type: {None (0), LZW (1), PACKBITS(2), DEFLATE (3), PS (4), CCITT G3 Fax (5), CCITT G4 Fax (6)}
		 )
	)
)

(script-fu-register
   "impactConvertPStoTIFF"
   "<Toolbox>/Xtns/Script-Fu/impact/impactConvertPStoTIFF" 
   "impact Convert PS to TIFF"
   "IMPACT"
